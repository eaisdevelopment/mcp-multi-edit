...
1. Add new `formatMultiEditResponse` function that:
   - Takes `MultiEditResult`, `includeContent: boolean`, and `totalEdits: number`
...
2. For SUCCESS responses, return:
   ```typescript
   {
     ...
     final_content?: string       // ONLY if includeContent=true (see below)
   }
   ```

   **Conditional final_content (CONTEXT.md locked decision):**
   - When `includeContent=true`: include `final_content` field with full file content
   - When `includeContent=false` or omitted: **omit the field entirely** (do NOT include as null/undefined)
   ...
   if (includeContent && result.finalContent) {
     response.final_content = result.finalContent;
   }
   return response;
   ```
...
3. For ERROR responses, return:
   ```typescript
   {
     success: false,
     file_path: string,
     error: string,              // formatted as "Edit N of M failed: reason"
     ...
     context_snippet?: string    // ~50 chars around expected match (for match errors)
   }
   ```

   **"Edit N of M failed" format (CONTEXT.md locked decision):**
   When formatting error messages, use pattern:
   ```typescript
   const errorMessage = `Edit ${failedEditIndex + 1} of ${totalEdits} failed: ${reason}`;
   ```
   This shows position of failed edit so Claude knows which edit to fix.
...
5. Add `extractContextSnippet(fileContent: string, searchString: string, radius: number = 25): string | undefined` function:

   **Context snippet extraction (CONTEXT.md locked decision):**
   For match errors, extract ~50 chars around expected match location to help Claude understand the mismatch.

   Implementation approach:
   - Find a partial match or similar location in file content (fuzzy match the first ~20 chars of searchString)
   - If found, extract `radius` chars before and after that position
   - Format as: `"...${before}[HERE]${after}..."`
   - If no partial match found, return first 50 chars of file with "File starts with: ..." prefix
   - Return undefined if file is empty

   This helps Claude see what's actually in the file near where it expected a match.

6. Add `truncateForDisplay(str: string, maxLen: number): string` helper.

Keep existing functions for backward compatibility (they're used by existing tests).
  </action>
  <verify>Run `npm run build` - should compile without errors</verify>
  <done>formatMultiEditResponse returns structured objects with: conditional final_content (omitted when false), "Edit N of M failed" error format, context snippets for match errors, recovery hints, and atomicity message</done>
</task>
...
1. Update `src/tools/multi-edit.ts`:
   ...
   - Update `handleMultiEdit` to:
     ...
     b. Call `formatMultiEditResponse(result, input.include_content ?? false, input.edits.length)`
...
5. Recovery hints exist in reporter:
   ...
   ```
6. Context snippet extraction exists:
   ```bash
   grep "extractContextSnippet\|context_snippet" src/core/reporter.ts
   ```
7. "Edit N of M" format in error handling:
   ```bash
   grep "Edit.*of.*failed" src/core/reporter.ts
   ```
</verification>
...
- Error responses include "Edit N of M failed" format with recovery hints and context snippets
- Success responses include final_content ONLY when include_content=true (omitted otherwise)
- Error responses include atomicity message
</success_criteria>
